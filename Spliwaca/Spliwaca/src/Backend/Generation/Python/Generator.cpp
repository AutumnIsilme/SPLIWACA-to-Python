...
namespace Spliwaca
{
	...
	std::shared_ptr<PythonGenerator> PythonGenerator::Create(std::shared_ptr<EntryPoint> entryPoint)
	{
		return std::make_shared<PythonGenerator>(entryPoint);
	}

	std::string PythonGenerator::GenerateCode(int& errorCode)
	{
		//PROFILE_FUNCTION();
	...
	void PythonGenerator::GenerateStatements(std::shared_ptr<Statements> statements)
	{
		for (std::shared_ptr<Statement> s : statements->statements)
		{
			PROFILE_SCOPE("Statement loop - PythonGenerator::GenerateStatements(std::shared_ptr<Statements>)");
	...
	void PythonGenerator::GenerateIf(std::shared_ptr<IfNode> node)
	{
		//PROFILE_FUNCTION();
	...
	void PythonGenerator::GenerateSet(std::shared_ptr<SetNode> node)
	{
		//PROFILE_FUNCTION();
	...
	void PythonGenerator::GenerateInput(std::shared_ptr<InputNode> node)
	{
		//PROFILE_FUNCTION();
		if (node->id->accessPresent)
		{
	...
	void PythonGenerator::GenerateOutput(std::shared_ptr<OutputNode> node)
	{
		//PROFILE_FUNCTION();
		m_Code += m_Tabs + "print(" + ParseRaw(node->raw) + ")";
	}

	void PythonGenerator::GenerateInc(std::shared_ptr<IncNode> node)
	{
		//PROFILE_FUNCTION();
		if (node->id->accessPresent)
		{
	...
	void PythonGenerator::GenerateDec(std::shared_ptr<DecNode> node)
	{
		//PROFILE_FUNCTION();
		if (node->id->accessPresent)
		{
	...
	void PythonGenerator::GenerateFor(std::shared_ptr<ForNode> node)
	{
		//PROFILE_FUNCTION();
		if (node->id->accessPresent)
		{
	...
	void PythonGenerator::GenerateWhile(std::shared_ptr<WhileNode> node)
	{
		//PROFILE_FUNCTION();
	...
	void PythonGenerator::GenerateQuit(std::shared_ptr<QuitNode> node)
	{
		//PROFILE_FUNCTION();
		m_Code += m_Tabs + "exit(";
		if (node->returnVal)
	...
	void PythonGenerator::GenerateCall(std::shared_ptr<CallNode> node, bool statement)
	{
		//PROFILE_FUNCTION();
		if (statement)
			m_Code += m_Tabs;
	...
	void PythonGenerator::GenerateFunc(std::shared_ptr<FuncNode> node)
	{
		//PROFILE_FUNCTION();
	...
	void PythonGenerator::GenerateProc(std::shared_ptr<ProcNode> node)
	{
		//PROFILE_FUNCTION();
		std::string func_name = "__func_name_line_" + std::to_string(node->id->GetLineNumber()) + "_char_" + std::to_string(node->id->GetColumnNumber());
		m_Code += m_Tabs + "@libsplw.type_check()\ndef " + func_name + "(prev_scope_vars: dict";
	...
	void PythonGenerator::GenerateStruct(std::shared_ptr<StructNode> node)
	{
		//PROFILE_FUNCTION();
	...
	void PythonGenerator::GenerateReturn(std::shared_ptr<ReturnNode> node)
	{
		//PROFILE_FUNCTION();
		m_Code += m_Tabs + "return "; GenerateList(node->list);
	}

	void PythonGenerator::GenerateImport(std::shared_ptr<ImportNode> node)
	{
		//PROFILE_FUNCTION();
		m_Code += m_Tabs + "import " + node->id->GetContents();
	}

	void PythonGenerator::GenerateList(std::shared_ptr<ListNode> node, bool fromAtom)
	{
	...
	void PythonGenerator::GenerateDictEntry(std::shared_ptr<DictEntryNode> node)
	{
	...
	void PythonGenerator::GenerateExpr(std::shared_ptr<Expr> node)
	{
	...
	void PythonGenerator::GenerateBinOp(std::shared_ptr<BinOpNode> node)
	{
	...
	void PythonGenerator::GenerateFactor(std::shared_ptr<FactorNode> node)
	{
	...
	void PythonGenerator::GenerateAtom(std::shared_ptr<AtomNode> node)
	{
		switch (node->type)
		{
		case 1:
	...
	void PythonGenerator::GenerateCreate(std::shared_ptr<CreateNode> node)
	{
	...
	void PythonGenerator::GenerateCast(std::shared_ptr<CastNode> node)
	{
		GenerateType(node->castType); m_Code += "("; GenerateList(node->list); m_Code += ")";
	}

	void PythonGenerator::GenerateAnonf(std::shared_ptr<AnonfNode> node)
	{
	...
	void PythonGenerator::GenerateAnonp(std::shared_ptr<AnonpNode> node)
	{
		uint32_t charIndex = (uint32_t)m_Code.size() - 1;
		std::string code = "";
		while (m_Code.at(charIndex) != '\n' && charIndex != -1)
	...
	void PythonGenerator::GenerateType(std::shared_ptr<TypeNode> node)
	{
		if (node->type == 1)
		{
	...
	/*void PythonGenerator::GenerateIdent(std::shared_ptr<IdentNode> node)
	{
	...
	std::string PythonGenerator::ParseRaw(std::shared_ptr<Token> token)
	{
		//PROFILE_FUNCTION();
		std::string code = "fr\"";
		bool inIdent = false;
		...
		for (char c : contents)
		{
			//PROFILE_SCOPE("Token contents loop - PythonGenerator::ParseRaw(std::shared_ptr<Token>)");
			if (inIdent)
			{
				//PROFILE_SCOPE("inIdent - PythonGenerator::ParseRaw(std::shared_ptr<Token>)");
				if (c == ' ')
				{
			...
			{
				SPLW_ERROR("We shouldn't be here: line 935, PythonGenerator.cpp!");
			}
		}
		if (inIdent)
		{
	...
	std::string PythonGenerator::ParseComplex(std::shared_ptr<Token> token)
	{
		std::string code = "";
	...
	std::string PythonGenerator::StripLeadingZeros(std::string token)
	{
